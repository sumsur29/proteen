...
const AURAS = {
  paneer_mafia: {
    name: "PANEER MAFIA", emoji: "🧀", color: "#FF5733", accent: "#FF7B5C",
    bg: "linear-gradient(160deg, #0A0200 0%, #1A0A05 40%, #25100A 100%)",
...
const PhoneMockup = ({ children, label }) => (
  <div style={{ flexShrink: 0, scrollSnapAlign: "center", width: 185 }}>
    <div style={{ width: 185, height: 370, background: "#0A0A0F", borderRadius: 28, border: "2px solid rgba(255,255,255,0.08)", position: "relative", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.03)" }}>
      ...
      <div style={{ padding: "28px 14px 6px", display: "flex", justifyContent: "space-between", fontSize: 9, color: "rgba(255,255,255,0.4)", fontWeight: 600 }}><span>9:41</span><span>●●● ■</span></div>
      <div style={{ padding: "0 12px 12px", height: 310, overflow: "hidden" }}>{children}</div>
    </div>
  ...
  <div style={{ textAlign: "center" }}>
    ...
    <div style={{ position: "relative", width: 100, height: 100, margin: "8px auto" }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ fontSize: 7, color: "#FF5733", fontWeight: 700 }}>TOP 15%</div>
      </div>
    </div>
    ...
    <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "8px 10px", textAlign: "left" }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#fff", marginBottom: 4 }}><span>Today</span><span style={{ color: "#FF5733" }}>62g ✓</span></div>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "rgba(255,255,255,0.4)" }}><span>Target</span><span>80g</span></div>
    </div>
    <div style={{ background: "#FF5733", borderRadius: 8, padding: 6, marginTop: 8, fontSize: 10, fontWeight: 800, color: "#fff" }}>SHARE SCORE →</div>
...
const RoastScreen = () => (
  <div>
    <div style={{ fontSize: 10, color: "#FF5733", fontWeight: 700, marginBottom: 8, letterSpacing: 1 }}>🔥 ROAST MODE</div>
    <div style={{ background: "rgba(255,87,51,0.08)", border: "1px solid rgba(255,87,51,0.12)", borderRadius: 12, padding: 10, marginBottom: 8 }}>
      ...
      <div style={{ fontSize: 11, color: "#FF5733", marginTop: 6, fontWeight: 700 }}>Sharma ji disappointed hai. 😔</div>
    </div>
    <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: 8 }}>
      ...
      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", lineHeight: 1.8 }}>☕ Chai + biscuit — 2g<br/>🥟 3x samosa — 9g<br/>🍚 Rice + dal — 8g</div>
      <div style={{ fontSize: 10, color: "#FF5733", marginTop: 6, fontWeight: 600 }}>Total: 19g / 80g target 💀</div>
    </div>
  </div>
);
...
const WrappedScreen = () => (
  <div style={{ textAlign: "center" }}>
    <div style={{ fontSize: 9, color: "#D4A853", fontWeight: 700, letterSpacing: 1, marginBottom: 4 }}>PROTEIN WRAPPED</div>
    ...
    <div style={{ background: "rgba(212,168,83,0.08)", borderRadius: 8, padding: 8, textAlign: "left" }}>
      <div style={{ fontSize: 9, color: "rgba(255,255,255,0.25)", marginBottom: 3 }}>YOU'RE IN THE</div>
      <div style={{ fontSize: 22, fontWeight: 900, color: "#D4A853" }}>TOP 12%</div>
      <div style={{ fontSize: 8, color: "rgba(255,255,255,0.25)" }}>of India 🇮🇳</div>
    </div>
    <div style={{ background: "#D4A853", borderRadius: 8, padding: 6, marginTop: 8, fontSize: 10, fontWeight: 800, color: "#000" }}>SHARE WRAPPED →</div>
  </div>
);
...
export default function ProteeenQuiz() {
  ...
  const generateAndAction = useCallback(async (action) => {
    ...
    ctx.beginPath(); ctx.roundRect(barX, barY, (result.rizzScore/100)*barW, barH, 18); ctx.fill();
    ctx.font = "bold 16px sans-serif"; ctx.fillStyle = "#fff";
    ctx.fillText(`RIZZ: ${result.rizzScore}/100`, barX + (result.rizzScore/100)*barW/2, barY + 24);
    const statsY = 660;
    [{icon:"⚡",label:"PROTEIN",val:result.proteinLevel},{icon:"🏆",label:"PERCENTILE",val:`TOP ${100-result.percentile}%`},{icon:"🔥",label:"RIZZ",val:`${result.rizzScore}/100`}].forEach((s,i) => {
      ...
      ctx.font = "12px sans-serif"; ctx.fillStyle = "rgba(255,255,255,0.25)"; ctx.fillText(s.label, sx, statsY+60);
  ...
  // PALETTE
  const P = { primary: "#FF5733", gold: "#D4A853", bg: "#08080C", cream: "#F5F0EB", red: "#EF4444" };
  const ft = "'Anybody', Impact, sans-serif";
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
    ...
    @keyframes glitch{0%,100%{text-shadow:2px 0 ${P.primary},-2px 0 ${P.gold}}25%{text-shadow:-2px 0 ${P.primary},2px 0 ${P.gold}}50%{text-shadow:2px 2px ${P.primary},-2px -2px ${P.gold}}75%{text-shadow:-2px 2px ${P.primary},2px -2px ${P.gold}}}
    ...
    <div style={pg}><style>{css}</style>
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, background: P.primary, padding: "7px 0", overflow: "hidden", zIndex: 10 }}>
        <div style={{ display: "flex", animation: "marquee 25s linear infinite", whiteSpace: "nowrap" }}>
          {Array(10).fill("PROTEEEN • APP COMING SOON • INDIA KA PROTEIN APP • ").map((t, i) => (
            <span key={i} style={{ fontSize: 11, fontWeight: 900, color: "#fff", fontFamily: ft, letterSpacing: 2 }}>{t}</span>
          ))}
        </div>
      </div>
      <div style={{ textAlign: "center", maxWidth: 520, zIndex: 2, animation: "fadeUp 0.8s ease", marginTop: 40, padding: "0 4px" }}>
        ...
        <button onClick={() => setStep("diet")} style={{
          background: `linear-gradient(135deg, ${P.primary}, #FF7B5C)`, color: "#fff", border: "none", borderRadius: 16, padding: "18px 48px",
          fontSize: "clamp(1rem,4vw,1.15rem)", fontWeight: 900, fontFamily: ft, textTransform: "uppercase", letterSpacing: 1,
          boxShadow: `0 8px 40px ${P.primary}30`, animation: "pulse 2.5s infinite",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[
            ...
            onMouseLeave={e => { e.currentTarget.style.borderColor = d.color + "20"; e.currentTarget.style.transform = "translateX(0)"; }}>
              ...
              <div>
                <div style={{ fontSize: 16, fontWeight: 900, color: d.color, fontFamily: ft, letterSpacing: 1 }}>{d.label}</div>
    ...
    <div style={{ ...pg, background: result.bg, justifyContent: "flex-start", paddingTop: 30, paddingBottom: 40 }}><style>{css}</style>
      ...
      <div style={{ textAlign: "center", maxWidth: 440, zIndex: 2, animation: "fadeUp 0.6s ease" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 18, justifyContent: "center" }}>
          {[
            { label: "PROTEIN", value: result.proteinLevel, color: result.proteinLevel === "VERY HIGH" ? "#34D399" : result.proteinLevel === "HIGH" ? P.gold : "#FB7185" },
    ...
    <div style={pg}><style>{css}</style>
      <div style={{ textAlign: "center", maxWidth: 440, animation: "fadeUp 0.5s ease", zIndex: 2, padding: "0 4px" }}>
        {submitted ? (<>
          ...
          {result && (
            <button onClick={() => generateAndAction("download")} style={{ width: "100%", background: `linear-gradient(135deg, ${P.primary}, #FF7B5C)`, color: "#fff", border: "none", borderRadius: 14, padding: 16, fontSize: 15, fontWeight: 900, fontFamily: ft, textTransform: "uppercase", letterSpacing: 1, marginBottom: 12 }}>📥 DOWNLOAD & SHARE MY AURA</button>
          )}
          ...
          <h1 style={{ fontSize: "clamp(2.5rem,10vw,4rem)", fontFamily: ft, fontWeight: 900, marginBottom: 6 }}>
